(function (factory) {
  ...
  */
  ...
  var Landscape = /*#__PURE__*/function () {
    ...
    function Landscape(props, generate) {
      ...
      this.props = Object.assign({
        ...
        random: prng.random
      }, props); // Defining a zero ground Landscape result in a full-height mono ground mode
      // This MONO_MODE may be used by graphic designers for specific tasks

      this.MONO_MODE = this.props.groundsLength <= 1;
    ...
    _proto.generate = function generate() {
      ...
      for (var index = 0; index < Math.max(groundsLength, 1); index++) {
        _loop(index);
      }
  ...
  var COLORS = [{
    ...
    color.css = "rgb(" + color.rgb + ")"; // Graphic designers define CMYK values on 0~100 range, but W3C defines them as 0~1 range

    color.cmyk = color.cmyk.map(function (v) {
      return v / 100;
  ...
  function proof(_ref) {
    ...
    var Y = 255 * (1 - y);
    var K = 255 * (1 - k);
    var r = 80 + 0.5882 * C - 0.3529 * M - 0.1373 * Y + 0.00185 * C * M + 0.00046 * Y * C;
    var g = 66 - 0.1961 * C + 0.2745 * M - 0.0627 * Y + 0.00215 * C * M + 0.00008 * Y * C + 0.00062 * Y * M;
    var b = 86 - 0.3255 * C - 0.1569 * M + 0.1647 * Y + 0.00046 * C * M + 0.00123 * Y * C + 0.00215 * Y * M;
    return [r * K / 255, g * K / 255, b * K / 255].map(function (v) {
      return n(v, 0, 255);
    });
  }
  ...
  function clone(color) {
    return Object.assign({}, color, {
      rgb: [].concat(color.rgb),
      cmyk: [].concat(color.cmyk)
    });
  }
  ...
  function swatch(_temp) {
    var _ref2 = _temp === void 0 ? {} : _temp,
        ...
        colorSpace = _ref2$colorSpace === void 0 ? null : _ref2$colorSpace,
        _ref2$random = _ref2.random,
        random = _ref2$random === void 0 ? prng.random : _ref2$random;
  ...
  var erode = (function (landscape, canvas, _temp) {
    ...
    function build() {
      if (landscape.MONO_MODE) return; // Compute breakpoints at which a line will be swaped with the next
  ...
  var canvas = (function (landscape, canvas, _temp) {
    var _ref = _temp === void 0 ? {} : _temp,
        _ref$colorSpace = _ref.colorSpace,
        colorSpace = _ref$colorSpace === void 0 ? 'css' : _ref$colorSpace;
  ...
  document.title += ' | ' + prng.seed; // Colors.log(Colors.toArray(), 8)

  var c = Colors.toArray().map(function (c) {
    return c['css-cmyk-proof'];
  });
  Colors.log(c, 8); // Landscape generation, agnostic of any rendering context
